...
inquirer
    .prompt([
        {// text is the input the user enters for what 1-3 letters are going to show up on the logo
            ...
            validate: (input) => {//validate checks if the provided function returns true and if not allows the user to reattempt until it does
                if(specialCharRegex.test(input)) return 'Your input cannot contain special characters'
        ...
        },
        {
            type: 'input',
            name: 'textColor',
            message: 'Please enter the color you want the text on your logo to be in either a valid CSS color or in hexadecimal lead by a "#"',
            validate: (input) => {
    ...
    .then((data) => {
        ...
        switch (data.shape) {
            case 'Circle':
                const circle = new Circle(data.text, data.textColor, data.color)
            ...
            case 'Triangle':
                const triangle = new Triangle(data.text, data.textColor, data.color)
            ...
            case 'Square':
                const square = new Square(data.text, data.textColor, data.color)
